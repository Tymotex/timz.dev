...
published: true
date: 2022-06-25
thumbnail: /images/thumbnails/canary-bird.png
tags:
    - Cybersecurity
    - Computer Science
---
...
A **buffer** in the context of a buffer overflow attack is basically a fixed-length memory region such as an integer array or string that the program writes data to. A **buffer overflow** is when the program tries to write more elements into that array than its allocated size, causing the program to silently write the data to adjacent memory addresses, overwriting their contents. 
...
<Image src="/images/blogs/concepts/Buffer-Overflow-Attacks/buffer-overflow-example.png" alt="Buffer overflow example" width="100%" />
...
<Image src="/images/blogs/concepts/Buffer-Overflow-Attacks/stack-canarying.png" alt="Stack canarying diagram" />
...
Often however, stack canarying can be performed behind the scenes for you by the compiler (see gcc’s [fstack-protector](https://linux.die.net/man/1/gcc) option) and the chosen canary value will be a randomised value specific to each process.
...
<Image src="/images/blogs/concepts/Buffer-Overflow-Attacks/canary-bird.png" alt="Cute canary birds" width="50%" shadow />

## Closing Note

There are plenty of other buffer overflow defence strategies as well as attack strategies that overcome them.

* [Prevent execution of code on the stack](https://en.wikipedia.org/wiki/Stack_buffer_overflow).
